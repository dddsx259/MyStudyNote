...
+ Divide:
  ...
  + Partition the array into three parts: 
    ...
    + Elements equal to the pivot $R$
    + Elements greater than the pivot $M$
+ Conquer:
...
+ $$T(n) = T(\frac{n}{2}) + O(n) \Rightarrow T(n) = O(n)$$
...
+ Combine:
  + Only need to consider points within a distance of $\min(d_L, d_R)$ from the dividing line.
    + i.e. $x\in [mid - \min(d_L, d_R), mid + \min(d_L, d_R)]$
...
+ Divide: Split each number into two halves. For example, if we have two n-digit numbers $X$ and $Y$, we can represent them as:
  + $X = a \cdot 2^{m} + b$
  + $Y = c \cdot 2^{m} + d$      
...
+ $$T(n) = 3T(\frac{n}{2}) + O(n) \Rightarrow T(n) = O(n^{\log_2 3}) \approx O(n^{1.585})$$
...
+ $$T(n) = 7T(\frac{n}{2}) + O(n^2) \Rightarrow T(n) = O(n^{\log_2 7}) \approx O(n^{2.81})$$
...
+ Divide:
  ...
  + $$p(x) = p_{odd}(x^2)+ x\cdot p_{even}(x), q(x) = q_{odd}(x^2)+ x\cdot q_{even}(x)$$
  + Recursively compute $(pq)(\omega_n^k)\text{ for } k = 0, 1, \ldots, n$, where n is the degree of the resulting polynomial $pq$.
  + Note that the n-root of unity $\omega$ satisfies $(\omega_n^k)^2 = \omega_n^{2k} = \omega_{n/2}^k$
  + So we have:
    + $$pq(\omega_n^k) = p_{odd}(\omega_{n/2}^k)q_{odd}(\omega_{n/2}^k) + \omega_n^k p_{even}(\omega_{n/2}^k)q_{even}(\omega_{n/2}^k)$$
    + $$pq(\omega_n^{k+n/2}) = p_{odd}(\omega_{n/2}^k)q_{odd}(\omega_{n/2}^k) - \omega_n^k p_{even}(\omega_{n/2}^k)q_{even}(\omega_{n/2}^k)$$
  + This requires three multiplications of n/2-degree polynomials: $p_{odd}q_{odd}$, $p_{even}q_{even}$, and $(p_{odd}+p_{even})(q_{odd}+q_{even})$.
+ Combine: 
  + We can get $n$ discitct values of $pq$ at the n-th roots of unity, which can be used to interpolate the coefficients of $pq$ in O(n) time using the inverse FFT.
+ $$T(n) = 3T(\frac{n}{2}) + O(n) \Rightarrow T(n) = O(n^{\log_2 3}) \approx O(n^{1.585})$$
...
+ Divide:
  ...
  + $$p(x) = p_{odd}(x^2)+ x\cdot p_{even}(x)$$
+ Conquer:
  + Recursively compute $y_{odd} = p_{odd}(\omega_{n/2}^k)$ and $y_{even} = p_{even}(\omega_{n/2}^k)$ for $k = 0, 1, \ldots, n/2 - 1$.
  + Note that the n-root of unity $\omega$ satisfies $(\omega_n^k)^2 = \omega_n^{2k} = \omega_{n/2}^k$.
  + So we have:
    + $$y_k = p_{odd}(\omega_{n/2}^k) + \omega_n^k p_{even}(\omega_{n/2}^k) = y_{odd}[k] + \omega_n^k y_{even}[k]$$
    + $$y_{k+n/2} = p_{odd}(\omega_{n/2}^k) - \omega_n^k p_{even}(\omega_{n/2}^k) = y_{odd}[k] - \omega_n^k y_{even}[k]$$
...
+ Advanced greedy: 
  ...
  + Procedure:
    ...
    + For each item in the sorted order:
      ...
      + If the item's weight $w_i$ is greater than the remaining capacity, add as much of the item as possible to fill the knapsack, update $total\_value$ accordingly, and break out of the loop.
...
> [Compression and Complexity](../Data%20Structure%28UCB%20CS61B%29/Markdown/Compression%20and%20Complexity.md)
