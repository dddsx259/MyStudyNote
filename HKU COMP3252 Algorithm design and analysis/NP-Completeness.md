...
+ *Conjunctive Normal Form (CNF)*: 
  ...
  + The more information about CNF can be found in the [SAT](../UCB%20CS188/10.%20Logic.md) page.
  + Proof:
...
+ **Subset Sum Problem**:
   + Given a set of integers, is there a non-empty subset that sums to zero?
   + + Proof1:
     + We can reduce the 3SAT Problem to the Subset Sum Problem by:
       1. Suppose we have a 3-SAT formula $F$ with $m$ clauses and $n$ variables.
       2. We assign a unique integer to each literal in the 3-SAT formula, such that the integers corresponding to the literals in the same clause sum to zero.
       3. We set the target sum to be zero.
       4. If there is a satisfying assignment for the 3-SAT formula, then there is a non-empty subset of the integers that sums to zero. Conversely, if there is a non-empty subset of the integers that sums to zero, then there is a satisfying assignment for the 3-SAT formula.
       5. Therefore, we have $3-SAT \leq_p Subset\ Sum$.
   + Proof2:
     + We can reduce the Vertex Cover Problem to the Subset Sum Problem by:
       1. Suppose we have a graph $G$ and an integer $k$ for the Vertex Cover Problem.
       2. We assign a unique prime number to each vertex in $G$.
       3. For each edge in $G$, we create an integer that is the product of the prime numbers corresponding to the vertices incident to that edge.
       4. We set the target sum to be the product of the prime numbers corresponding to all vertices in $G$ divided by the product of the prime numbers corresponding to the vertices in the vertex cover.
       5. If there is a vertex cover of size at most $k$ in $G$, then there is a non-empty subset of the integers that sums to zero. Conversely, if there is a non-empty subset of the integers that sums to zero, then there is a vertex cover of size at most $k$ in $G$.
       6. Therefore, we have $Vertex\ Cover \leq_p Subset\ Sum$.
...
+ **Hamiltonian Path Problem**:
   ...
   + Proof:
     + We can reduce the 3-SAT Problem to the Hamiltonian Path Problem by:
       1. Suppose we have a 3-SAT formula $F$ with $m$ clauses and $n$ variables.
       2. We construct a graph $G$ with $2n + m$ vertices, where each variable corresponds to two vertices (one for true and one for false) and each clause corresponds to one vertex.
       3. We connect the vertices corresponding to the literals in the same clause to the vertex corresponding to that clause.
       4. We also connect the vertices corresponding to the literals of the same variable to each other.
       5. If there is a satisfying assignment for the 3-SAT formula, then there is a Hamiltonian path in $G$ that visits each vertex exactly once. Conversely, if there is a Hamiltonian path in $G$ that visits each vertex exactly once, then there is a satisfying assignment for the 3-SAT formula.
       6. Therefore, we have $3-SAT \leq_p Hamiltonian\ Path$.
...
+ **Undirected Hamiltonian Cycle Problem**:
   ...
   + Proof:
     + We can reduce the Directed Hamiltonian Cycle Problem to the Undirected Hamiltonian Cycle Problem by:
       ...
       2. We construct a new undirected graph $G'$ by replacing each directed edge in $G$ with an undirected edge.
